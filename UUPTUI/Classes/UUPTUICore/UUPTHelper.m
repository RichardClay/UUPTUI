...
//  UUPT
//
//  Created by UUPT Team on 14/10/25.
//
...
NSString *const kUUPTResourcesBundleName = @"UUPTResources";

@implementation UUPTHelper (Bundle)

+ (UIImage *)imageWithName:(NSString *)name {
    static NSBundle *resourceBundle = nil;
    if (!resourceBundle) {
        NSBundle *mainBundle = [NSBundle bundleForClass:self];
        NSString *resourcePath = [mainBundle pathForResource:kUUPTResourcesBundleName ofType:@"bundle"];
...
@implementation UUPTHelper (DynamicType)
...
+ (CGFloat)heightForDynamicTypeCell:(NSArray *)heights {
    NSNumber *index = [UUPTHelper preferredContentSizeLevel];
    return [((NSNumber *)[heights objectAtIndex:[index intValue]]) UUPT_CGFloatValue];
}
@end

@implementation UUPTHelper (Keyboard)

UUPTSynthesizeBOOLProperty(keyboardVisible, setKeyboardVisible)
UUPTSynthesizeCGFloatProperty(lastKeyboardHeight, setLastKeyboardHeight)

- (void)handleKeyboardWillShow:(NSNotification *)notification {
    self.keyboardVisible = YES;
    self.lastKeyboardHeight = [UUPTHelper keyboardHeightWithNotification:notification];
}
...
+ (BOOL)isKeyboardVisible {
    BOOL visible = [UUPTHelper sharedInstance].keyboardVisible;
    return visible;
}

+ (CGFloat)lastKeyboardHeightInApplicationWindowWhenVisible {
    return [UUPTHelper sharedInstance].lastKeyboardHeight;
}
...
+ (CGFloat)keyboardHeightWithNotification:(NSNotification *)notification {
    return [UUPTHelper keyboardHeightWithNotification:notification inView:nil];
}
...
+ (UIViewAnimationOptions)keyboardAnimationOptionsWithNotification:(NSNotification *)notification {
    UIViewAnimationOptions options = [UUPTHelper keyboardAnimationCurveWithNotification:notification]<<16;
    return options;
}
...
@implementation UUPTHelper (AudioSession)
...
@implementation UUPTHelper (UIGraphic)
...
@implementation UUPTHelper (Device)
        ...
        if (isNotchedScreen < 0) {
            if (@available(iOS 12.0, *)) {
                /*
                 ...
                 2. 与触发 [UUPTHelper isNotchedScreen] 方法时的进程有关，例如 https://github.com/Tencent/UUPT_iOS/issues/482#issuecomment-456051738 里提到的 [NSObject performSelectorOnMainThread:withObject:waitUntilDone:NO] 就会导致较多的异常。
                ...
                [[UIScreen mainScreen] UUPT_performSelector:peripheryInsetsSelector withPrimitiveReturnValue:&peripheryInsets];
                ...
                isNotchedScreen = [UUPTHelper is58InchScreen] ? 1 : 0;
            }
        }
    } else {
        isNotchedScreen = 0;
...
+ (BOOL)is65InchScreen {
    if (is65InchScreen < 0) {
        ...
        is65InchScreen = (DEVICE_WIDTH == self.screenSizeFor65Inch.width && DEVICE_HEIGHT == self.screenSizeFor65Inch.height && ([[UUPTHelper deviceModel] isEqualToString:@"iPhone11,4"] || [[UUPTHelper deviceModel] isEqualToString:@"iPhone11,6"] || [[UUPTHelper deviceModel] isEqualToString:@"iPhone12,5"])) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreen {
    if (is61InchScreen < 0) {
        is61InchScreen = (DEVICE_WIDTH == self.screenSizeFor61Inch.width && DEVICE_HEIGHT == self.screenSizeFor61Inch.height && ([[UUPTHelper deviceModel] isEqualToString:@"iPhone11,8"] || [[UUPTHelper deviceModel] isEqualToString:@"iPhone12,1"])) ? 1 : 0;
    }
...
+ (CGFloat)preferredLayoutAsSimilarScreenWidthForIPad {
    if (preferredLayoutWidth < 0) {
        ...
        for (NSInteger i = 0; i < widths.count; i++) {
            if (windowWidth <= widths[i].UUPT_CGFloatValue) {
                preferredLayoutWidth = widths[i].UUPT_CGFloatValue;
                continue;
            }
        }
    }
...
+ (BOOL)isHighPerformanceDevice {
    if (isHighPerformanceDevice < 0) {
        NSString *model = [UUPTHelper deviceModel];
        NSString *identifier = [model UUPT_stringMatchedByPattern:@"\\d+"];
...
@implementation UUPTHelper (UIApplication)
...
@implementation UUPTHelper (Animation)
...
@implementation UUPTHelper (SystemVersion)
...
+ (BOOL)isCurrentSystemAtLeastVersion:(NSString *)targetVersion {
    return [UUPTHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedSame || [UUPTHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedDescending;
}

+ (BOOL)isCurrentSystemLowerThanVersion:(NSString *)targetVersion {
    return [UUPTHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedAscending;
}

@end

@implementation UUPTHelper

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        [UUPTHelper sharedInstance];// 确保内部的变量、notification 都正确配置
    });
}

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static UUPTHelper *instance = nil;
    dispatch_once(&onceToken,^{
...
- (void)dealloc {
    // UUPTHelper 若干个分类里有用到消息监听，所以在 dealloc 的时候注销一下

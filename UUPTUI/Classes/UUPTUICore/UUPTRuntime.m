...
//  UUPTKit
//
//  Created by UUPT Team on 2018/9/5.
//
...
@implementation UUPTPropertyDescriptor

+ (instancetype)descriptorWithProperty:(objc_property_t)property {
    UUPTPropertyDescriptor *descriptor = [[UUPTPropertyDescriptor alloc] init];
    ...
    descriptor.type = [UUPTPropertyDescriptor typeWithEncodeString:[NSString stringWithUTF8String:type]];
    if (type != NULL) {
        free(type);
    }
...
int UUPT_getProjectClassList(classref_t **classes) {
    size_t count = 0;
    if (!!classes) {

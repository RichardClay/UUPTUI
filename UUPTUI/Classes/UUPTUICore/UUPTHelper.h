...
@interface UUPTHelper : NSObject

+ (instancetype)sharedInstance;
@end

@interface UUPTHelper (Bundle)

/// 获取 UUPTKit.framework Images.xcassets 内的图片资源
/// @param name 图片名
+ (nullable UIImage *)imageWithName:(NSString *)name;

@end

@interface UUPTHelper (SystemVersion)
...
@interface UUPTHelper (DynamicType)
...
/**
 * 获取当前键盘在屏幕上的可见高度，注意外接键盘（iPad那种）时，[UUPTHelper keyboardRectWithNotification]得到的键盘rect里有一部分是超出屏幕，不可见的，如果直接拿rect的高度来计算就会与意图相悖。
 * @param notification 接收到的键盘事件的UINotification对象
 * @param view 要得到的键盘高度是相对于哪个View的键盘高度，若为nil，则等同于调用[UUPTHelper keyboardHeightWithNotification:]
...
@interface UUPTHelper (AudioSession)
...
@interface UUPTHelper (UIGraphic)
...
@interface UUPTHelper (UIApplication)
 ...
 * @note 在 iOS 13 以前  UIStatusBarStyleDefault 状态栏内容的颜色固定是黑色的，而在 iOS 13 UIStatusBarStyleDefault 会根据 user interface style 来决定状态栏的颜色，如果你需要一直黑色可以用 UUPTStatusBarStyleDarkContent 来代替以前 UIStatusBarStyleDefault 的写法
...
@interface UUPTHelper (Animation)

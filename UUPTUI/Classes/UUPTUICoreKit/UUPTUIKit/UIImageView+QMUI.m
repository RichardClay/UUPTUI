...
//  UUPT
//
//  Created by UUPT Team on 16/8/9.
//
...
@implementation UIImageView (UUPT)

UUPTSynthesizeIdStrongProperty(qimgv_animatedImageLayer, setQimgv_animatedImageLayer)
UUPTSynthesizeIdStrongProperty(qimgv_displayLink, setQimgv_displayLink)
UUPTSynthesizeIdStrongProperty(qimgv_animatedImage, setQimgv_animatedImage)
UUPTSynthesizeNSIntegerProperty(qimgv_currentAnimatedImageIndex, setQimgv_currentAnimatedImageIndex)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UIImageView class], @selector(setImage:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIImageView *selfObject, UIImage *image) {
                ...
                if (selfObject.UUPT_smoothAnimation && image.images) {
...
- (BOOL)qimgv_requestToStartAnimation {
    ...
    self.qimgv_displayLink.paused = self.UUPT_pause;
...
- (BOOL)qimgv_canStartAnimation {
    return self.UUPT_visible && !CGRectIsEmpty(self.frame);
}
...
- (void)setUUPT_smoothAnimation:(BOOL)UUPT_smoothAnimation {
    objc_setAssociatedObject(self, &kAssociatedObjectKey_smoothAnimation, @(UUPT_smoothAnimation), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (UUPT_smoothAnimation && self.image.images && self.image != self.qimgv_animatedImage) {
        self.image = self.image;// 重新设置图片，触发动画
    } else if (!UUPT_smoothAnimation && self.qimgv_animatedImage) {
        self.image = self.image;// 交给 setImage 那边把动画清理掉
    }
}

- (BOOL)UUPT_smoothAnimation {
...
- (void)setUUPT_pause:(BOOL)UUPT_pause {
    ...
    if (self.animationImages || self.image.images) {
        self.qimgv_animatedImageLayer.UUPT_pause = UUPT_pause;
    }
    if (self.qimgv_displayLink) {
        self.qimgv_displayLink.paused = UUPT_pause;
    }
}
...
- (void)UUPT_sizeToFitKeepingImageAspectRatioInSize:(CGSize)limitSize {
    if (!self.image) {
        return;
    }

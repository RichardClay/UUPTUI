...
@protocol UUPTConfigurationTemplateProtocol <NSObject>
...
@optional
/// 当返回 YES 时，启动 App 的时候 UUPTConfiguration 会自动应用这份配置表。但启动 App 时自动应用的配置表最多只允许一份，如果有多份则其他的会被忽略
/// UUPTConfiguration automatically applies this template on launch when set to YES. Since only one copy of configuration template is allowed when the app launches, you'll have to call `applyConfigurationTemplate` manually if you have more than one configuration templates. 
- (BOOL)shouldApplyTemplateAutomatically;

@end

/**
 *  维护项目全局 UI 配置的单例，通过业务项目自己的 UUPTConfigurationTemplate 来为这个单例赋值，而业务代码里则通过 UUPTConfigurationMacros.h 文件里的宏来使用这些值。
 *  A singleton that contains various UI configurations. Use `UUPTConfigurationTemplate` to set values; Use macros in `UUPTConfigurationMacros.h` to get values.
 */
@interface UUPTConfiguration : NSObject
...
#pragma mark - Project
#pragma mark -颜色
@property (nonatomic,strong)UIColor *C_Title_Text; //1A1A1A
@property (nonatomic,strong)UIColor *C_Primary_Text; //333333
@property (nonatomic,strong)UIColor *C_Normal_Text; //666666
@property (nonatomic,strong)UIColor *C_Prompt_Cell; //8A8A8A
@property (nonatomic,strong)UIColor *C_Prompt_Text; //999999
@property (nonatomic,strong)UIColor *C_Disable_Text; //CCCCCC

@property (nonatomic,strong)UIColor *C_Brand; //FF8B03
@property (nonatomic,strong)UIColor *C_Warning; //FF2828

@property (nonatomic,strong)UIColor *C_Main_Background; //F2F2F2
@property (nonatomic,strong)UIColor *C_View_Background; //F8F8F8
@property (nonatomic,strong)UIColor *C_Btn_Diable; //DBDBDB
@property (nonatomic,strong)UIColor *C_Dividing_Line; //E5E5E5

@property (nonatomic,assign)CGFloat MaskAlpa; //0.4

#pragma mark -字体相关
@property (nonatomic,strong)UIFont *F1M;
@property (nonatomic,strong)UIFont *F1R; //Font 24
@property (nonatomic,strong)UIFont *F2M;
@property (nonatomic,strong)UIFont *F2R; //Font 18
@property (nonatomic,strong)UIFont *F3M;
@property (nonatomic,strong)UIFont *F3R; //Font 16
@property (nonatomic,strong)UIFont *F4M;
@property (nonatomic,strong)UIFont *F4R; //Font 15
@property (nonatomic,strong)UIFont *F5M;
@property (nonatomic,strong)UIFont *F5R; //Font 14
@property (nonatomic,strong)UIFont *F6M;
@property (nonatomic,strong)UIFont *F6R; //Font 13
@property (nonatomic,strong)UIFont *F7M;
@property (nonatomic,strong)UIFont *F7R; //Font 12
@property (nonatomic,strong)UIFont *F8M;
@property (nonatomic,strong)UIFont *F8R; //Font 11
@property (nonatomic,strong)UIFont *F9M;
@property (nonatomic,strong)UIFont *F9R; //Font 10
...
@property(nonatomic, assign) CGFloat            windowLevelUUPTAlertView;
...
@property(nonatomic, assign) BOOL               shouldPrintUUPTWarnLogToConsole;
@property(nonatomic, assign) BOOL               sendAnalyticsToUUPTTeam;
...
@interface UITabBarItem (UUPTConfiguration)

- (void)UUPT_updateTintColorForiOS12AndEarlier:(nullable UIColor *)tintColor;
@end

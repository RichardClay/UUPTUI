...
NSString *const UUPTThemeDidChangeNotification = @"UUPTThemeDidChangeNotification";
...
@implementation UUPTThemeManager

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static UUPTThemeManager *instance = nil;
...
// 这个方法的声明放在 UUPTThemeManagerCenter.m 里，简单实现 private 的效果
- (instancetype)initWithName:(__kindof NSObject<NSCopying> *)name {
    if (self = [super init]) {
        ...
        if (@available(iOS 13.0, *)) {
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleUserInterfaceStyleWillChangeNotification:) name:UUPTUserInterfaceStyleWillChangeNotification object:nil];
        }
    }
    return self;
}
...
- (void)setCurrentThemeIdentifier:(NSObject<NSCopying> *)currentThemeIdentifier {
    ...
    NSAssert([self._themeIdentifiers containsObject:currentThemeIdentifier], @"%@ should be added to UUPTThemeManager.themes before it becomes current theme identifier.", currentThemeIdentifier);
...
- (void)setCurrentTheme:(NSObject *)currentTheme {
    ...
    NSAssert([self._themes containsObject:currentTheme], @"%@ should be added to UUPTThemeManager.themes before it becomes current theme.", currentTheme);
...
- (void)notifyThemeChanged {
    [[NSNotificationCenter defaultCenter] postNotificationName:UUPTThemeDidChangeNotification object:self];
    ...
    [UIApplication.sharedApplication.windows enumerateObjectsUsingBlock:^(__kindof UIWindow * _Nonnull window, NSUInteger idx, BOOL * _Nonnull stop) {
        if (!window.hidden && window.alpha > 0.01 && window.rootViewController) {
            [window.rootViewController UUPT_themeDidChangeByManager:self identifier:self.currentThemeIdentifier theme:self.currentTheme];
            if (window.rootViewController.isViewLoaded) {
                [window.rootViewController.view _UUPT_themeDidChangeByManager:self identifier:self.currentThemeIdentifier theme:self.currentTheme shouldEnumeratorSubviews:YES];
            }
        }
    }];
}

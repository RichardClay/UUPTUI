...
@implementation UIView (UUPTTheme)

UUPTSynthesizeIdCopyProperty(UUPT_themeDidChangeBlock, setUUPT_themeDidChangeBlock)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        // iOS 12 及以下的版本，[UIView setBackgroundColor:] 并不会保存传进来的 color，所以要自己用个变量保存起来，不然 UUPTThemeColor 对象就会被丢弃
        if (@available(iOS 13.0, *)) {
            ...
            ExtendImplementationOfVoidMethodWithSingleArgument([UIView class], @selector(setBackgroundColor:), UIColor *, ^(UIView *selfObject, UIColor *color) {
                selfObject.UUPTTheme_backgroundColor = color;
            });
            ExtendImplementationOfNonVoidMethodWithoutArguments([UIView class], @selector(backgroundColor), UIColor *, ^UIColor *(UIView *selfObject, UIColor *originReturnValue) {
                return selfObject.UUPTTheme_backgroundColor ?: originReturnValue;
            });
        }
        ...
        OverrideImplementation([UIView class], @selector(setHidden:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIView *selfObject, BOOL firstArgv) {
                ...
                if (valueChanged) {
                    // UIView.UUPT_currentThemeIdentifier 只是为了实现判断当前的 theme 是否有发生变化，所以可以构造成一个 string，但怎么避免每次 hidden 切换时都要遍历所有的 subviews？
                    [selfObject _UUPT_themeDidChangeByManager:nil identifier:nil theme:nil shouldEnumeratorSubviews:YES];
                }
            };
        });
        ...
        [classes enumerateObjectsUsingBlock:^(Class  _Nonnull class, NSUInteger idx, BOOL * _Nonnull stop) {
            ExtendImplementationOfVoidMethodWithoutArguments(class, @selector(didMoveToWindow), ^(UIView *selfObject) {
                ...
                if (selfObject.window) {
                    [selfObject _UUPT_themeDidChangeByManager:nil identifier:nil theme:nil shouldEnumeratorSubviews:NO];
                }
            });
        }];
    });
...
- (void)UUPT_registerThemeColorProperties:(NSArray<NSString *> *)getters {
    [getters enumerateObjectsUsingBlock:^(NSString * _Nonnull getterString, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (!self.UUPTTheme_themeColorProperties) {
            self.UUPTTheme_themeColorProperties = NSMutableDictionary.new;
        }
        self.UUPTTheme_themeColorProperties[getterString] = setterString;
    }];
}

- (void)UUPT_unregisterThemeColorProperties:(NSArray<NSString *> *)getters {
    if (!self.UUPTTheme_themeColorProperties) return;
    
    [getters enumerateObjectsUsingBlock:^(NSString * _Nonnull getterString, NSUInteger idx, BOOL * _Nonnull stop) {
        [self.UUPTTheme_themeColorProperties removeObjectForKey:getterString];
    }];
}
...
- (void)UUPT_themeDidChangeByManager:(UUPTThemeManager *)manager identifier:(__kindof NSObject<NSCopying> *)identifier theme:(__kindof NSObject *)theme {
    if (![self _UUPT_visible]) return;
    
    // 常见的 view 在 UUPTThemePrivate 里注册了 getter，在这里被调用
    [self.UUPTTheme_themeColorProperties enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull getterString, NSString * _Nonnull setterString, BOOL * _Nonnull stop) {
        ...
        if (getter == @selector(tintColor)) {
            if (!self.UUPT_tintColorCustomized) return;
        }
        ...
        if (!value) return;
        BOOL isValidatedColor = [value isKindOfClass:UUPTThemeColor.class] && (!manager || [((UUPTThemeColor *)value).managerName isEqual:manager.name]);
        BOOL isValidatedImage = [value isKindOfClass:UUPTThemeImage.class] && (!manager || [((UUPTThemeImage *)value).managerName isEqual:manager.name]);
        BOOL isValidatedEffect = [value isKindOfClass:UUPTThemeVisualEffect.class] && (!manager || [((UUPTThemeVisualEffect *)value).managerName isEqual:manager.name]);
    ...
    // 特殊的 view 特殊处理
    // iOS 10-11 里当 UILabel.attributedText 的文字颜色都相同时，也无法使用 setNeedsDisplay 刷新样式，但只要某个 range 颜色不同就没问题，iOS 9、12-13 也没问题，这个通过 UILabel (UUPTThemeCompatibility) 兼容。
    // iOS 9-13，当 UITextField 没有聚焦时，不需要调用 setNeedsDisplay 系统都可以自动更新文字样式，但聚焦时调用 setNeedsDisplay 也无法更新样式，这里依赖了 UITextField (UUPTThemeCompatibility) 对 setNeedsDisplay 做的兼容实现了更新
    // 注意，iOS 11 及以下的 UITextView 直接调用 setNeedsDisplay 是无法刷新文字样式的，这里依赖了 UITextView (UUPTThemeCompatibility) 里通过 swizzle 实现了兼容，iOS 12 及以上没问题。
    ...
    // 输入框、搜索框的键盘跟随主题变化
    if (UUPTCMIActivated && [self conformsToProtocol:@protocol(UITextInputTraits)]) {
    ...
    /** 这里去掉动画有 2 个原因：
     1. iOS 13 进入后台时会对 currentTraitCollection.userInterfaceStyle 做一次取反进行截图，以便在后台切换 Drak/Light 后能够更新 app 多任务缩略图，UUPT 响应了这个操作去调整取反后的 layer 的颜色，而在对 layer 设置属性的时候，如果包含了动画会导致截图不到最终的状态，这样会导致在后台切换 Drak/Light 后多任务缩略图无法及时更新。
     2. 对于 UIView 层，修改 backgroundColor 默认是没有动画的，而 CALayer 修改 backgroundColor 会有隐式动画，这里为了在响应主题变化时颜色同步更新，统一把 CALayer 的动画去掉
     */
    [CALayer UUPT_performWithoutAnimation:^{
        [self.layer UUPT_setNeedsUpdateDynamicStyle];
    }];
    
    if (self.UUPT_themeDidChangeBlock) {
        self.UUPT_themeDidChangeBlock();
    }
}

@end

@implementation UIView (UUPTTheme_Private)

UUPTSynthesizeIdStrongProperty(UUPTTheme_backgroundColor, setUUPTTheme_backgroundColor)
UUPTSynthesizeIdStrongProperty(UUPTTheme_themeColorProperties, setUUPTTheme_themeColorProperties)
...
- (void)_UUPT_themeDidChangeByManager:(UUPTThemeManager *)manager identifier:(__kindof NSObject<NSCopying> *)identifier theme:(__kindof NSObject *)theme shouldEnumeratorSubviews:(BOOL)shouldEnumeratorSubviews {
    [self UUPT_themeDidChangeByManager:manager identifier:identifier theme:theme];
    if (shouldEnumeratorSubviews) {
        [self.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull subview, NSUInteger idx, BOOL * _Nonnull stop) {
            [subview _UUPT_themeDidChangeByManager:manager identifier:identifier theme:theme shouldEnumeratorSubviews:YES];
        }];
    }
}

...
+ (UIImage *)UUPT_imageWithSize:(CGSize)size opaque:(BOOL)opaque scale:(CGFloat)scale actions:(void (^)(CGContextRef contextRef))actionBlock {
...
- (UIImage *)UUPT_grayImage {
    // CGBitmapContextCreate 是无倍数的，所以要自己换算成1倍
    CGSize size = self.UUPT_sizeInPixel;
    ...
    if (self.UUPT_opaque) {
        ...
        // 用 CGBitmapContextCreateImage 方式创建出来的图片，CGImageAlphaInfo 总是为 CGImageAlphaInfoNone，导致 UUPT_opaque 与原图不一致，所以这里再做多一步
        grayImage = [UIImage UUPT_imageWithSize:grayImage.size opaque:NO scale:grayImage.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)UUPT_imageWithAlpha:(CGFloat)alpha {
    return [UIImage UUPT_imageWithSize:self.size opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawInRect:CGRectMakeWithSize(self.size) blendMode:kCGBlendModeNormal alpha:alpha];
    }];
}

- (UIImage *)UUPT_imageWithTintColor:(UIColor *)tintColor {
...
//    }
//#endif
    return [UIImage UUPT_imageWithSize:self.size opaque:self.UUPT_opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)UUPT_imageWithBlendColor:(UIColor *)blendColor {
    UIImage *coloredImage = [self UUPT_imageWithTintColor:blendColor];
...
- (UIImage *)UUPT_imageWithImageAbove:(UIImage *)image atPoint:(CGPoint)point {
    return [UIImage UUPT_imageWithSize:self.size opaque:self.UUPT_opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)UUPT_imageWithSpacingExtensionInsets:(UIEdgeInsets)extension {
    ...
    return [UIImage UUPT_imageWithSize:contextSize opaque:self.UUPT_opaque scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawAtPoint:CGPointMake(extension.left, extension.top)];
    }];
}

- (UIImage *)UUPT_imageWithClippedRect:(CGRect)rect {
...
- (UIImage *)UUPT_imageWithClippedCornerRadius:(CGFloat)cornerRadius {
    return [self UUPT_imageWithClippedCornerRadius:cornerRadius scale:self.scale];
}

- (UIImage *)UUPT_imageWithClippedCornerRadius:(CGFloat)cornerRadius scale:(CGFloat)scale {
    if (cornerRadius <= 0) {
        return self;
    }
    return [UIImage UUPT_imageWithSize:self.size opaque:NO scale:scale actions:^(CGContextRef contextRef) {
        [[UIBezierPath bezierPathWithRoundedRect:CGRectMakeWithSize(self.size) cornerRadius:cornerRadius] addClip];
        [self drawInRect:CGRectMakeWithSize(self.size)];
    }];
}

- (UIImage *)UUPT_imageResizedInLimitedSize:(CGSize)size {
    return [self UUPT_imageResizedInLimitedSize:size resizingMode:UUPTImageResizingModeScaleAspectFit];
}

- (UIImage *)UUPT_imageResizedInLimitedSize:(CGSize)size resizingMode:(UUPTImageResizingMode)resizingMode {
    return [self UUPT_imageResizedInLimitedSize:size resizingMode:resizingMode scale:self.scale];
}

- (UIImage *)UUPT_imageResizedInLimitedSize:(CGSize)size resizingMode:(UUPTImageResizingMode)resizingMode scale:(CGFloat)scale {
    ...
    if (resizingMode >= UUPTImageResizingModeScaleAspectFit && resizingMode <= UUPTImageResizingModeScaleAspectFillBottom) {
        CGFloat horizontalRatio = size.width / imageSize.width;
        CGFloat verticalRatio = size.height / imageSize.height;
        CGFloat ratio = 0;
        if (resizingMode >= UUPTImageResizingModeScaleAspectFill && resizingMode < (UUPTImageResizingModeScaleAspectFill + 10)) {
            ratio = MAX(horizontalRatio, verticalRatio);
        } else {
            // 默认按 UUPTImageResizingModeScaleAspectFit
        ...
        // 默认按照 UUPTImageResizingModeScaleToFill
        drawingRect = CGRectMakeWithSize(size);
        contextSize = size;
    }
    
    return [UIImage UUPT_imageWithSize:contextSize opaque:self.UUPT_opaque scale:scale actions:^(CGContextRef contextRef) {
        [self drawInRect:drawingRect];
    }];
}

- (UIImage *)UUPT_imageWithOrientation:(UIImageOrientation)orientation {
    ...
    return [UIImage UUPT_imageWithSize:contextSize opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)UUPT_imageWithBorderColor:(UIColor *)borderColor path:(UIBezierPath *)path {
    if (!borderColor) {
        return self;
    }
...
- (UIImage *)UUPT_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius {
    return [self UUPT_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:cornerRadius dashedLengths:0];
}

- (UIImage *)UUPT_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius dashedLengths:(const CGFloat *)dashedLengths {
    ...
    return [self UUPT_imageWithBorderColor:borderColor path:path];
}

- (UIImage *)UUPT_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth borderPosition:(UUPTImageBorderPosition)borderPosition {
    if (borderPosition == UUPTImageBorderPositionAll) {
        return [self UUPT_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((UUPTImageBorderPositionBottom & borderPosition) == UUPTImageBorderPositionBottom) {
        ...
        if ((UUPTImageBorderPositionTop & borderPosition) == UUPTImageBorderPositionTop) {
            [path moveToPoint:CGPointMake(0, borderWidth / 2)];
            [path addLineToPoint:CGPointMake(self.size.width, borderWidth / 2)];
        }
        if ((UUPTImageBorderPositionLeft & borderPosition) == UUPTImageBorderPositionLeft) {
            [path moveToPoint:CGPointMake(borderWidth / 2, 0)];
            [path addLineToPoint:CGPointMake(borderWidth / 2, self.size.height)];
        }
        if ((UUPTImageBorderPositionRight & borderPosition) == UUPTImageBorderPositionRight) {
...
- (UIImage *)UUPT_imageWithMaskImage:(UIImage *)maskImage usingMaskImageMode:(BOOL)usingMaskImageMode {
...
+ (UIImage *)UUPT_animatedImageWithData:(NSData *)data {
    return [self UUPT_animatedImageWithData:data scale:1];
}

+ (UIImage *)UUPT_animatedImageWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self UUPT_frameDurationAtIndex:i source:source];
...
+ (float)UUPT_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)UUPT_animatedImageNamed:(NSString *)name {
    return [UIImage UUPT_animatedImageNamed:name scale:1];
}

+ (UIImage *)UUPT_animatedImageNamed:(NSString *)name scale:(CGFloat)scale {
    ...
    return [UIImage UUPT_animatedImageWithData:data scale:scale];
}

+ (UIImage *)UUPT_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size path:(UIBezierPath *)path addClip:(BOOL)addClip {
    size = CGSizeFlatted(size);
    return [UIImage UUPT_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)UUPT_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth cornerRadius:(CGFloat)cornerRadius {
    ...
    return [UIImage UUPT_imageWithStrokeColor:strokeColor size:size path:path addClip:NO];
}

+ (UIImage *)UUPT_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth borderPosition:(UUPTImageBorderPosition)borderPosition {
    CGContextInspectSize(size);
    if (borderPosition == UUPTImageBorderPositionAll) {
        return [UIImage UUPT_imageWithStrokeColor:strokeColor size:size lineWidth:lineWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((UUPTImageBorderPositionBottom & borderPosition) == UUPTImageBorderPositionBottom) {
...
+ (UIImage *)UUPT_imageWithColor:(UIColor *)color {
    return [UIImage UUPT_imageWithColor:color size:CGSizeMake(4, 4) cornerRadius:0];
}

+ (UIImage *)UUPT_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadius:(CGFloat)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    
    color = color ? color : UIColorClear;
	BOOL opaque = (cornerRadius == 0.0 && [color UUPT_alpha] == 1.0);
    return [UIImage UUPT_imageWithSize:size opaque:opaque scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)UUPT_imageWithShape:(UUPTImageShape)shape size:(CGSize)size lineWidth:(CGFloat)lineWidth tintColor:(UIColor *)tintColor {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage UUPT_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        switch (shape) {
            ...
            case UUPTImageShapeNavBack: {
                ...
                break;
            case UUPTImageShapeDisclosureIndicator: {
                drawByStroke = YES;
                path = [UIBezierPath bezierPath];
                path.lineWidth = lineWidth;
        ...
        if (shape == UUPTImageShapeDetailButtonImage) {
...
+ (UIImage *)UUPT_imageWithShape:(UUPTImageShape)shape size:(CGSize)size tintColor:(UIColor *)tintColor {
    CGFloat lineWidth = 0;
    switch (shape) {
        case UUPTImageShapeNavBack:
            lineWidth = 2.0f;
    ...
    return [UIImage UUPT_imageWithShape:shape size:size lineWidth:lineWidth tintColor:tintColor];
}

+ (UIImage *)UUPT_imageWithAttributedString:(NSAttributedString *)attributedString {
    ...
    return [UIImage UUPT_imageWithSize:stringSize opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [attributedString drawInRect:CGRectMakeWithSize(stringSize)];
    }];
}

+ (UIImage *)UUPT_imageWithView:(UIView *)view {
    CGContextInspectSize(view.bounds.size);
    return [UIImage UUPT_imageWithSize:view.bounds.size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [view.layer renderInContext:contextRef];
    }];
}

+ (UIImage *)UUPT_imageWithView:(UIView *)view afterScreenUpdates:(BOOL)afterUpdates {

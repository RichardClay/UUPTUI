...
- (NSArray<NSString *> *)UUPT_toTrimmedArray {
    return [[self UUPT_toArray] UUPT_filterWithBlock:^BOOL(NSString *item) {
        return item.UUPT_trim.length > 0;
    }];
}
...
- (NSString *)UUPT_trimAllWhiteSpace {
    return [self stringByReplacingOccurrencesOfString:@"\\s" withString:@"" options:NSRegularExpressionSearch range:NSMakeRange(0, self.length)];
}

- (NSString *)UUPT_trimLineBreakCharacter {
...
- (NSString *)UUPT_capitalizedString {
    if (self.length)
...
- (NSString *)UUPT_removeMagicalChar {
    if (self.length == 0) {
        return self;
    }
...
- (NSUInteger)UUPT_lengthWhenCountingNonASCIICharacterAsTwo {
    NSUInteger length = 0;
...
- (NSString *)UUPT_substringAvoidBreakingUpCharacterSequencesFromIndex:(NSUInteger)index {
    return [self UUPT_substringAvoidBreakingUpCharacterSequencesFromIndex:index lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)UUPT_substringAvoidBreakingUpCharacterSequencesToIndex:(NSUInteger)index {
    return [self UUPT_substringAvoidBreakingUpCharacterSequencesToIndex:index lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)UUPT_substringAvoidBreakingUpCharacterSequencesWithRange:(NSRange)range {
    return [self UUPT_substringAvoidBreakingUpCharacterSequencesWithRange:range lessValue:YES countingNonASCIICharacterAsTwo:NO];
}
...
- (NSString *)UUPT_stringByRemoveLastCharacter {
    return [self UUPT_stringByRemoveCharacterAtIndex:self.length - 1];
}
...
@implementation NSString (UUPT_StringFormat)
...
+ (instancetype)UUPT_stringWithCGFloat:(CGFloat)floatValue {
    return [NSString UUPT_stringWithCGFloat:floatValue decimal:2];
}

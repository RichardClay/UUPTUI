...
@interface QMUIPopContainerViewController : UIViewController
...
- (void)dealloc {
    _sourceView.UUPT_frameDidChangeBlock = nil;
}
...
- (void)layoutDefaultSubviews {
    self.contentView.frame = CGRectMake(self.borderWidth + self.contentEdgeInsets.left,
                                        (self.currentLayoutDirection == QMUIPopupContainerViewLayoutDirectionAbove ? self.borderWidth : self.arrowSize.height + self.borderWidth) + self.contentEdgeInsets.top,
                                        CGRectGetWidth(self.bounds) - self.borderWidth * 2 - UIEdgeInsetsGetHorizontalValue(self.contentEdgeInsets),
                                        CGRectGetHeight(self.bounds) - self.arrowSize.height - self.borderWidth * 2 - UIEdgeInsetsGetVerticalValue(self.contentEdgeInsets));
    ...
    if (isTextLabelShowing) {
        CGFloat textLabelMinX = self.textEdgeInsets.left;
        ...
        _textLabel.frame = CGRectMake(textLabelOrigin.x, textLabelOrigin.y, ceil(textLabelSize.width), ceil(textLabelSize.height));
    }
    if (isImageViewShowing) {
        [_imageView sizeToFit];
        _imageView.frame = CGRectMake(ceil(CGRectGetMaxX(_textLabel.frame) + self.textEdgeInsets.right) + self.imageEdgeInsets.left, CGFloatGetCenter(CGRectGetHeight(self.contentView.bounds), CGRectGetHeight(_imageView.frame) + self.imageEdgeInsets.top), _imageView.frame.size.width, _imageView.frame.size.height);
    }
...
- (void)setSourceView:(__kindof UIView *)sourceView {
    ...
    sourceView.UUPT_frameDidChangeBlock = ^(__kindof UIView * _Nonnull view, CGRect precedingFrame) {
        ...
        CGRect rect = [view UUPT_convertRect:view.bounds toView:convertToView];
        weakSelf.sourceRect = rect;
    };
    sourceView.UUPT_frameDidChangeBlock(sourceView, sourceView.frame);// update layout immediately
}
...
- (void)didInitialize {
    ...
    [self addClickActionWithContentView];
...
- (void)addClickActionWithContentView{

    [self addTarget:self action:@selector(daoWithTipsAction) forControlEvents:UIControlEventTouchUpInside];
}

- (void)daoWithTipsAction{
    if (self.isShowing) {
        [self hideWithAnimated:YES];
    }
    if (self.onlisenerWithClickTipsCompletion) {
        self.onlisenerWithClickTipsCompletion();
    }
}
